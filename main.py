...
def log(value, status='Telkomsel', color='[CC]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('alung '.format(self.proxy_host, self.proxy_port))
            ...
            self.log('anakna supriyadi   ''[CC]'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
    	 '[G1]''SUBSCRIBE YOUTUBE CHANNEL','[Y1]'
    	 '[G1]''all thing','[CC]'
    	 '=============================================','[CC]'
     'Remod   : all things','[Y1]'
     'Script  : RG&ILMUthings','[CC]'
     'Tested  : Jawa timur','[Y1]'
     'Sunek?  : on-of mode pesawat','[CC]'
    ])))    
